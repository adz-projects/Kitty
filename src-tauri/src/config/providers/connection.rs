...
pub async fn test_connection(profile: &ProviderProfile) -> Result<(), String> {
    match profile.provider_type.as_str() {
        ...
        // Fireworks/DeepInfra are plain OpenAI-compatible hosted endpoints —
        // same `/models` probe as openai/custom_openai, no dialect-specific
        // handling needed (see bigtiny/providers.rs's
        // `bigtiny_provider_target`, which already maps both to the
...
mod tests {
    ...
    async fn fireworks_reachable_with_no_key_stored_is_ok() {
        ...
        // Fireworks/DeepInfra both fell through to the `other => Err(...)`
        // catch-all before this dialect was added to the shared
        // `"openai" | "custom_openai" | ...` arm; this is the regression
        // guard for that.
        let mut server = mockito::Server::new_async().await;
        let _models = server
            .mock("GET", "/models")
