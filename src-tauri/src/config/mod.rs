...
pub struct Config {
    ...
    /// Per-**model** reasoning-effort memory: `"<provider id>\0<model>"` → the
    /// wire value last confirmed for that pair, so choosing a model again
    /// brings its effort back instead of resetting to the dialect default.
    ///
    /// Keyed by provider *and* model rather than model alone, matching
    /// `bigtiny::effort::effort_cache_key`: the same model id served by two
    /// endpoints can expose different level sets (a self-hosted server's levels
    /// come from its own chat template), so a bare model key would let one
    /// endpoint's choice leak into another's dropdown as an option it never
    /// offered.
    ///
    /// Written only by `bigtiny::effort::confirm_model_effort`, on a chat's
    /// first turn, and only when the value actually differs from what's stored
    /// — a dropdown fiddled with and then abandoned before sending doesn't
    /// rewrite the model's remembered default.
    #[serde(default)]
    pub model_efforts: HashMap<String, String>,
    /// Per-**model** context-window memory: `"<provider id>\0<model>"` -> the
    /// window discovered for that pair, in tokens. Same key shape and the same
    /// reasoning as `model_efforts` above.
    ///
    /// This exists because the daemon's wrap-up valve reads the window off the
    /// provider row Kitty pushes, falling back to its own 64k default when the
    /// row has none — so an undiscovered window makes the valve withdraw tools
    /// far too early on a large model. Kitty could already *suggest* the value
    /// in the Providers form; remembering it here is what makes it automatic
    /// for a profile whose owner never opened that form.
    ///
    /// Written only by `bigtiny::context_window`, and never for a profile that
    /// carries an explicit `context_length` — that is a deliberate override and
