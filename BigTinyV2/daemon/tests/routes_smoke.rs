...
async fn test_state_inner(
    ...
    if let Some(engine) = pathway.clone() {
        plugins.insert_for_test(TEST_APP, engine).await;
    }
    ...
    // Every session these tests create is owned by this app. Registered for
    // real (rather than faked) so ownership checks resolve against a genuine
    // `apps` row.
    bigtiny2::storage::apps::register_app(&pool, TEST_APP, "Test App", "test-key")
        .await
        .unwrap();
...
/// The app id every session in these tests belongs to.
const TEST_APP: &str = "test-app";

/// `create_router` plus the identity the auth middleware would have resolved.
///
/// These tests exercise handlers, not authentication -- auth itself is covered
/// by `server::middleware`'s unit tests, and cross-tenant isolation by
/// `tests/tenancy.rs`. Injecting the extension directly keeps this file about
/// route behaviour while still going through the real ownership checks.
fn create_router(state: Arc<AppState>) -> axum::Router {
    router_as(state, TEST_APP)
}

/// Same, but as an arbitrary app -- used to prove a second app cannot reach
/// the first app's sessions.
fn router_as(state: Arc<AppState>, app_id: &str) -> axum::Router {
    use axum::Extension;
    use bigtiny2::storage::apps::AppIdentity;
    let identity = AppIdentity {
        app_id: app_id.to_string(),
        scopes: vec!["*".to_string()],
    };
    bigtiny2::routes::create_router(state).layer(Extension(identity))
}
...
async fn fork_remaps_the_compaction_boundary_to_the_new_sessions_own_rowids() {
    ...
    let boundary_rowid: i64 =
        sqlx::query_scalar("SELECT rowid FROM messages WHERE session_id = ? AND content = 'msg2'")
            ...
            .await
            .unwrap();
    bigtiny2::storage::sessions::update_compaction_state(
        &state.db,
        &session_id,
        "a summary of msg1",
        boundary_rowid,
...
async fn mcp_server_header_value_is_encrypted_at_rest_and_never_echoed_over_http() {
    ...
    assert_eq!(
        bigtiny2::crypto::decrypt(stored_headers_json["Authorization"].as_str().unwrap()),
        plaintext_key
    );
}
