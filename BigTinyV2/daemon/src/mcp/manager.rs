...
impl MCPManager {
    pub fn new(
        ...
        Self::with_data_dir(pool, pathway, std::path::PathBuf::new())
    }

    /// As [`Self::new`], but knowing the daemon's data dir so stdio servers can
    /// be given a per-app `KITTY_PLUGIN_HOME`. See `scoped_env`.
    ...
    async fn connect_server_locked(&self, server_id: &str) -> Result<(), MCPServerError> {
        ...
        let config = row_to_config(&row, &self.data_dir);
...
/// Give a stdio server owned by an app its own `KITTY_PLUGIN_HOME`.
///
/// `kitty-tools` resolves its home **once per process** from the environment
/// (`plugins/kitty-tools/src/paths.rs`), and everything it keeps -- the
/// scratchpad, the extract-once document cache -- hangs off that. Two apps
/// sharing one connection would therefore share a scratchpad, which is
/// somewhere between surprising and a data leak.
///
/// Pointing each app's child at `apps/<app_id>/plugin-home` fixes it for the
/// case that matters, because a stdio server is a separate process per row and
/// so can have a separate environment.
///
/// **Windows only, by design.** The in-process transport Android uses cannot
/// do this -- one process, one environment -- but Android hosts exactly one
/// app, so process-global *is* app-scoped there. This is a deliberate platform
/// difference, not deferred work.
///
/// A shared server (`app_id IS NULL`) keeps the daemon's own environment: it
/// belongs to no app, so there is no per-app home to give it.
fn scoped_env(
    data_dir: &std::path::Path,
    app_id: Option<&str>,
    env: Option<serde_json::Value>,
) -> Option<serde_json::Value> {
    let (Some(app_id), false) = (app_id, data_dir.as_os_str().is_empty()) else {
        return env;
    };
    let home = data_dir.join("apps").join(app_id).join("plugin-home");
    // Best-effort: a server whose home cannot be created still starts, it just
    // falls back to the process default rather than failing to connect.
    ...
    // Only when the row has not set one itself: an explicit value in the
...
fn row_to_config(row: &mcp_servers::MCPServerRow, data_dir: &std::path::Path) -> MCPServerConfig {
    ...
    MCPServerConfig {
        ...
        env: scoped_env(
            data_dir,
            row.app_id.as_deref(),
            row.env.as_ref().and_then(|s| serde_json::from_str(s).ok()),
        ),
        headers: row
            .headers
            .as_ref()
...
mod scoped_env_tests {
    use super::scoped_env;
    use serde_json::json;

    fn home_of(v: &Option<serde_json::Value>) -> Option<String> {
        v.as_ref()?
            .get("KITTY_PLUGIN_HOME")?
            .as_str()
            .map(str::to_owned)
    }

    #[test]
    fn two_apps_get_different_plugin_homes() {
        // The isolation this exists for: `kitty-tools` resolves its home once
        // per process, so a shared home means a shared scratchpad.
        let dir = tempfile::tempdir().unwrap();
        let a = scoped_env(dir.path(), Some("app-a"), None);
        let b = scoped_env(dir.path(), Some("app-b"), None);
        assert_ne!(home_of(&a), home_of(&b));
        assert!(home_of(&a).unwrap().contains("app-a"));
    }

    #[test]
    fn a_shared_server_keeps_the_daemon_environment() {
        // It belongs to no app, so there is no per-app home to give it.
        let dir = tempfile::tempdir().unwrap();
        assert!(scoped_env(dir.path(), None, None).is_none());
    }

    #[test]
    fn an_explicit_value_in_the_row_wins() {
        // An operator who set this by hand outranks our default.
        let dir = tempfile::tempdir().unwrap();
        let env = Some(json!({"KITTY_PLUGIN_HOME": "/operator/choice"}));
        assert_eq!(
            home_of(&scoped_env(dir.path(), Some("app-a"), env)),
            Some("/operator/choice".to_string())
        );
    }

    #[test]
    fn other_env_entries_are_preserved() {
        let dir = tempfile::tempdir().unwrap();
        let out = scoped_env(dir.path(), Some("app-a"), Some(json!({"FOO": "bar"}))).unwrap();
        assert_eq!(out["FOO"], "bar");
        assert!(out.get("KITTY_PLUGIN_HOME").is_some());
    }

    #[test]
    fn an_empty_data_dir_changes_nothing() {
        // Tests and in-process hosts pass no data dir; they must be untouched.
        let env = Some(json!({"FOO": "bar"}));
        let out = scoped_env(std::path::Path::new(""), Some("app-a"), env.clone());
        assert_eq!(out, env);
    }
}
