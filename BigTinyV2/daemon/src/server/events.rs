...
pub struct SSEEvent {
    ...
    /// The id of the `ToolCall` this frame belongs to, on `ToolStart`/
    /// `ToolFinish`.
    ///
    /// A step's tool calls run concurrently (`agent::loop_::execute_tools`), so
    /// several starts arrive before any finish and a client has no way to pair
    /// them by arrival order — it would attribute a result to whichever card
    /// happened to start last. Without this a long call in a batch (an
    /// `await_specialists` alongside its `call_specialist`s, say) leaves a card
    /// pending forever. Optional so an older client simply ignores it.
    #[serde(skip_serializing_if = "Option::is_none")]
