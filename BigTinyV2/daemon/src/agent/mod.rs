...
pub struct Agent {
    ...
    cache_dir: String,
    /// Loop-integrated plugins, hosted per app. The loop asks this for the
    /// engine belonging to the session's owning app rather than holding one
    /// daemon-wide handle, which is what keeps two frontends' belief graphs
    /// separate.
    ///
    /// This replaces V1's `pathway` + `pathway_shutdown` pair: each instance's
...
impl Agent {
    ...
    /// Whether any turn is currently in flight.
    ///
    /// Read by the idle-exit timer. A long generation makes no HTTP requests
    /// while it runs, so the activity clock alone would call an actively
    /// working daemon idle and shut it down mid-answer.
    ///
    /// Finished-but-unreaped entries are ignored rather than counted: the
    /// slot is only removed by `TurnCleanup`'s `Drop`, and a task that has
    /// completed but not yet been dropped is not work in progress.
