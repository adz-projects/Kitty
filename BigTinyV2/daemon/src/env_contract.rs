...
/// V2's data directory: `BIGTINYV2_DATA_DIR`, else `%APPDATA%/BigTinyV2` on
/// Windows, else `~/.bigtiny-v2`.
///
/// **Deliberately not V1's `BIGTINY_DATA_DIR` / `~/.bigtiny`.** Both daemons
/// run on the same machine through the migration, and sharing a data dir means
/// two daemons opening the same SQLite file with different migration chains:
/// V2 would run `017_apps_and_tenancy.sql` against a database V1 then no longer
/// understands. This was not hypothetical — the fork inherited V1's resolution
/// and, on its first run, published its handshake into `~/.bigtiny/` and
/// migrated the database it found there. Keep these paths apart.
///
/// V1's variable is deliberately *not* read as a fallback: a developer with
/// `BIGTINY_DATA_DIR` still exported for V1 would otherwise point V2 straight
/// at the database this separation exists to protect.
pub fn resolve_data_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("BIGTINYV2_DATA_DIR") {
    ...
    if cfg!(windows) {
        if let Some(appdata) = std::env::var_os("APPDATA") {
            return PathBuf::from(appdata).join("BigTinyV2");
        }
    }
    dirs_home().join(".bigtiny-v2")
}
...
#[cfg(test)]
mod tests {
    /// The separation that keeps V2 from migrating V1's database out from
    /// under it. Regression for the fork's first run, which published into
    /// `~/.bigtiny/` and applied migration 017 to whatever it found there.
    #[test]
    fn the_data_dir_is_never_v1s() {
        let prev = std::env::var_os("BIGTINYV2_DATA_DIR");
        std::env::remove_var("BIGTINYV2_DATA_DIR");
        // V1's variable must not be honoured, even when it is set.
        std::env::set_var("BIGTINY_DATA_DIR", "/v1/data/dir");
        let dir = super::resolve_data_dir();
        std::env::remove_var("BIGTINY_DATA_DIR");
        if let Some(p) = prev {
            std::env::set_var("BIGTINYV2_DATA_DIR", p);
        }

        let shown = dir.to_string_lossy().replace('\\', "/");
        assert!(!shown.contains("/v1/data/dir"), "V2 honoured V1's env var: {shown}");
        assert!(
            !shown.ends_with("/.bigtiny"),
            "V2 resolved to V1's default directory: {shown}"
        );
        assert!(
            shown.contains("BigTinyV2") || shown.ends_with(".bigtiny-v2"),
            "unexpected V2 data dir: {shown}"
        );
    }
