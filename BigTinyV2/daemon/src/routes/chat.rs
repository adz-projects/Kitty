...
/// Reject the request unless the calling app owns this session.
///
/// Returns `Some(response)` to short-circuit; `None` means "carry on". Every
/// `/api/chat/{id}/*` handler calls this first, and `tests/tenancy.rs` walks
/// them all cross-tenant so a new route that forgets it fails there rather
/// than leaking in production.
///
/// **404, not 403.** "This exists but is not yours" is itself a disclosure --
/// it confirms another app's session id -- and no legitimate caller benefits
...
async fn deny_unless_owned(
    ...
    match sessions::is_owned_by(&state.db, session_id, &identity.app_id).await {
        Ok(true) => None,
        Ok(false) => Some(err_response(
            StatusCode::NOT_FOUND,
            format!("no such session: {session_id}"),
        )),
        // Fail closed: a database error must not read as "owned".
...
pub async fn create_session(
    ...
    // Ownership is assigned at birth, never patched on afterwards: a session
    // with no owner is unreachable through every scoped accessor, i.e. a row
    // nothing can read, list, or delete.
    if let Err(e) = sessions::create_session_for_app(&state.db, &id, &name, &identity.app_id).await
    {
...
pub async fn rename_session(
    ...
    match sessions::update_session_name_for_app(&state.db, &id, &body.name, &identity.app_id).await
    {
...
pub async fn compact_session(
    ...
    // Scoped to the calling app, matching `run_tool_loop`'s resolution. An
    // unscoped call here would size compaction's budget against a provider
    // this app cannot even use.
    let app_default = crate::storage::apps::get_app(&state.db, &identity.app_id)
        .await
        .ok()
        .flatten()
    ...
    let resolved_provider_id = state
        .agent
        .router()
        .resolve_provider_for_app(&identity.app_id, effective_provider, app_default.as_deref())
        .ok();
