...
pub async fn create_provider(
    ...
    // Private to the creating app by default. `"shared": true` opts into
    // the pool every app can see -- the escape hatch for a user who would
    // rather enter one API key than one per frontend. Private is the
    // default because the safe mistake is a provider too few apps can see,
    // not one too many.
...
pub async fn update_provider(
    ...
    let existing = match providers::get_provider_for_app(&state.db, &id, &identity.app_id).await
    {
        Ok(Some(row)) => row,
    ...
    // Mutation is stricter than visibility. An app may *use* a shared
    // provider but not reconfigure one: rewriting the base URL or
    // credentials of a row every other app is routing through is not one
    // client's call to make.
    if existing.app_id.as_deref() != Some(identity.app_id.as_str()) {
        return err_response(
            StatusCode::FORBIDDEN,
            "shared providers cannot be modified by an individual app",
        );
    }
    ...
    let result = async {
        ...
        if let Some(priority) = priority {
            sqlx::query(
                r#"UPDATE providers SET fallback_priority = ? WHERE id = ? AND app_id = ?"#,
            )
        ...
        sqlx::query(r#"UPDATE providers SET name = COALESCE(?1, name), base_url = COALESCE(?2, base_url), config = COALESCE(?3, config), updated_at = CURRENT_TIMESTAMP WHERE id = ?4 AND app_id = ?5"#)
            .bind(name)
            .bind(base_url)
            .bind(&config)
            .bind(&id)
...
pub async fn delete_provider(
    ...
    match providers::delete_provider_owned(&state.db, &id, &identity.app_id).await {
        // 0 rows means it does not exist, belongs to another app, or is
        // shared -- none of which this caller may delete, and all of which
        // must read identically from the outside.
        Ok(0) => err_response(StatusCode::NOT_FOUND, "provider not found"),
        Ok(_) => {
...
pub async fn test_provider(
    ...
    // Probing a provider you cannot see would confirm it exists, and
    // would spend another app's endpoint (and rate limit) doing it.
    if !state.router.is_visible_to(&id, &identity.app_id) {
        return err_response(StatusCode::NOT_FOUND, "provider not found");
    }
