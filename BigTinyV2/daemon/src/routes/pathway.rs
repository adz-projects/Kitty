...
/// `result_large_err`.
///
/// Per app: a belief browser must show the caller its *own* graph. In V1 there
/// was one graph and so no question to ask; here, resolving the wrong app's
/// engine would show one frontend the inferences another had drawn about the
/// user.
async fn engine(
    state: &AppState,
