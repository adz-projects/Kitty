...
/// Reject the request unless the calling app may *see* this server (its own,
/// or a shared one).
async fn deny_unless_visible(
    ...
    match mcp_servers::get_server_for_app(&state.db, server_id, &identity.app_id).await {
        Ok(Some(_)) => None,
        Ok(None) => Some(err_response(
            StatusCode::NOT_FOUND,
            format!("no such MCP server: {server_id}"),
        )),
        // Fail closed: a database error must not read as "visible".
...
/// Reject the request unless the calling app *owns* this server.
///
/// Stricter than visibility, and deliberately so: a shared server's command
/// string is what every app's agent loop executes, so reconfiguring or
/// reconnecting one is not any single client's call.
async fn deny_unless_owned(
    state: &AppState,
    server_id: &str,
    identity: &AppIdentity,
) -> Option<Response> {
    if let Some(denied) = deny_unless_visible(state, server_id, identity).await {
        return Some(denied);
    }
    match mcp_servers::owner_of(&state.db, server_id).await {
        Ok(Some(owner)) if owner == identity.app_id => None,
        Ok(_) => Some(err_response(
            StatusCode::FORBIDDEN,
            "shared MCP servers cannot be modified by an individual app",
        )),
        Err(e) => Some(err_response(
            StatusCode::INTERNAL_SERVER_ERROR,
            e.to_string(),
...
pub async fn list_servers(
    ...
    match mcp_servers::list_servers_for_app(&state.db, &identity.app_id).await {
        Ok(rows) => {
...
pub async fn create_server(
    ...
    // Private to the registering app by default; `"shared": true` opts
    // into a server every app can use. Shared MCP servers are mutable by
    // nobody through the API -- the command string here is what every
    // app's agent loop executes, so repointing it is not one client's call.
    let owner: Option<&str> = if body.get("shared").and_then(|v| v.as_bool()) == Some(true) {
        None
    } else {
        Some(identity.app_id.as_str())
    };
    ...
    let result = async {
        ...
        sqlx::query(
            r#"INSERT INTO mcp_servers (id, name, transport, app_id) VALUES (?, ?, ?, ?)"#,
        )
            .bind(&id)
            .bind(name)
            .bind(transport)
...
pub async fn delete_server(
    ...
    // Ownership FIRST, disconnect second. The other order let any app
    // disconnect any other app's live server -- the DELETE would then fail to
    // remove the row and report success, but the tool set was already gone: a
    // cross-app denial of service with a 200 on it.
    if let Some(denied) = deny_unless_owned(&state, &id, &identity).await {
        return denied;
    }
    state.mcp.disconnect_server(&id).await;
    match mcp_servers::delete_server_owned(&state.db, &id, &identity.app_id).await {
        // 0 rows means it vanished between the check and the delete. A miss is
        // a 404, never a cheerful `{"ok": true}` for work that did not happen.
        Ok(0) => err_response(StatusCode::NOT_FOUND, format!("no such MCP server: {id}")),
...
pub async fn connect_server(
    ...
    // Owned, not merely visible: connecting spawns the row's command, and
    // a shared server's lifecycle is the daemon's to manage.
    if let Some(denied) = deny_unless_owned(&state, &id, &identity).await {
        return denied;
    }
...
pub async fn list_tools(
    ...
    // Read-only, so visibility is enough -- but not less than that: the
    // tool list names another app's capabilities and arguments.
    if let Some(denied) = deny_unless_visible(&state, &id, &identity).await {
        return denied;
    }
