...
use crate::tools::viz::layout::{
    draw_node, size_node_capped, NodeVisual, GAP_X, MAX_CONTENT_W_WIDE, MAX_NODE_W, MIN_LAYER_GAP,
    MIN_NODE_W,
};
...
pub fn render(steps: &[Step]) -> (String, f32, f32) {
    ...
    for _ in 0..6 {
        ...
        cap = (cap
            * (MAX_CONTENT_W_WIDE - gutter - LEFT_MARGIN - MIN_LAYER_GAP * (n as f32 - 1.0))
            / nodes_sum)
            .clamp(MIN_NODE_W, MAX_NODE_W);
        sized = steps
            .iter()
            .map(|s| size_node_capped(&s.text, None, cap))
            .collect();
    ...
    for i in 0..n.saturating_sub(1) {
        ...
        let mid_x = (x1 + x2) / 2.0;
        let d = format!(
            "M {x1:.1},{y1:.1} L {mid_x:.1},{y1:.1} L {mid_x:.1},{y2:.1} L {x2:.1},{y2:.1}"
        );
...
mod tests {
    ...
    fn renders_caller_supplied_lanes_and_steps() {
        let steps = vec![
            step("Place order", "Customer"),
            step("Charge card", "Payments"),
            step("Ship item", "Warehouse"),
        ];
        ...
        assert!(
            !body.contains("Submit Cart"),
            "must not fall back to the retired e-commerce clipart"
        );
        assert!(
            !body.contains("BACKEND API"),
            "must not fall back to the retired e-commerce clipart"
        );
    }
    ...
    fn every_node_and_divider_stays_within_canvas_bounds() {
        let steps: Vec<Step> = (0..8)
            .map(|i| {
                step(
                    &format!("Step {i}"),
                    if i % 2 == 0 { "Customer" } else { "Backend" },
                )
            })
            .collect();
