...
use kitty_tools::tools::viz::{
    generate_accessible_chart, generate_accessible_svg, generate_accessible_table,
};
...
fn assert_snapshot(name: &str, actual: &str) {
    ...
    let expected = fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!("no snapshot at {path:?} -- run with KITTY_UPDATE_SNAPSHOTS=1 to create it")
    });
...
fn single_lane_snapshot() {
    ...
    let out = generate_accessible_svg(
        DiagramType::SingleLane,
        "Ingestion pipeline",
        "A three-step data ingestion pipeline.",
        steps,
    );
...
fn flowchart_snapshot() {
    let steps = vec![
        ...
        Step {
            id: Some("c".into()),
            text: "Ship order".into(),
            step_type: StepType::End,
            ..Default::default()
        },
        Step {
            id: Some("d".into()),
            text: "Cancel order".into(),
            step_type: StepType::End,
            ..Default::default()
        },
    ];
    let out = generate_accessible_svg(
        ...
        "Order fulfillment",
        "How an order is fulfilled or cancelled.",
        steps,
    );
...
fn tree_snapshot() {
    let steps = vec![
        Step {
            id: Some("ceo".into()),
            text: "CEO".into(),
            next: vec!["eng".into(), "sales".into()],
            ..Default::default()
        },
        Step {
            id: Some("eng".into()),
            text: "VP Engineering".into(),
            ..Default::default()
        },
        Step {
            id: Some("sales".into()),
    ...
    let out = generate_accessible_svg(
        DiagramType::Tree,
        "Org chart",
        "Reporting structure for the leadership team.",
        steps,
    );
...
fn swimlane_snapshot() {
    let steps = vec![
        Step {
            ...
            lane: Some("Customer".into()),
            ..Default::default()
        },
        Step {
            text: "Charge card".into(),
            lane: Some("Payments".into()),
            ..Default::default()
        },
        Step {
            text: "Ship item".into(),
            lane: Some("Warehouse".into()),
            ..Default::default()
        },
    ];
    let out = generate_accessible_svg(
        ...
        "Order handoff",
        "Which team handles each step of an order.",
        steps,
    );
...
fn journey_map_snapshot() {
    let steps = vec![
        ...
        Step {
            ...
            subtitle: Some("Runs their first query".into()),
            sentiment: Some(2),
            ..Default::default()
        },
    ];
    let out = generate_accessible_svg(
        ...
        "Onboarding journey",
        "How a new user experiences onboarding.",
        steps,
    );
...
fn chart_snapshot() {
    ...
    let series = vec![ChartSeries {
        ...
        values: vec![12.4, 15.1, 22.8, 24.0],
    }];
...
fn table_snapshot() {
    ...
    let out = generate_accessible_table(
        "Regional sales",
        &headers,
        &rows,
        Some("Sales rose in both regions."),
    );
