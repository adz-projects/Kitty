...
fn node_boxes_never_overlap_in_any_diagram_type() {
    let cases: Vec<(DiagramType, Vec<Step>)> = vec![
        (
            ...
            (0..6)
                .map(|i| {
                    step(
                        &format!("s{i}"),
                        &format!("Process step number {i} with a label"),
                        &[],
                    )
                })
                .collect(),
        ...
        (
            ...
            (0..8)
                .map(|i| Step {
                    text: format!("Step {i}"),
                    lane: Some(if i % 2 == 0 { "Customer" } else { "Backend" }.to_string()),
                    ..Default::default()
                })
                .collect(),
        ),
    ...
    for (dtype, steps) in cases {
        ...
        let rects: Vec<Rect> = parse_rects(&html)
            .into_iter()
            .filter(|r| r.class.contains("node-box"))
            .collect();
        ...
        for (i, a) in rects.iter().enumerate() {
            for b in &rects[i + 1..] {
                assert!(
                    !overlaps(a, b),
                    "{dtype:?}: node rects overlap: {a:?} vs {b:?}"
                );
            }
        }
    }
...
fn swimlane_nodes_stay_below_their_lane_headers() {
    ...
    let steps: Vec<Step> = (0..6)
        .map(|i| Step {
            text: format!(
                "Step {i} with a deliberately long description that wraps to three lines for sure"
            ),
            lane: Some("Customer".to_string()),
            ..Default::default()
        })
        .collect();
...
fn skip_level_edges_are_rejected_with_a_hint() {
    ...
    assert_eq!(
        v["error_code"], "VIZ_LONG_EDGE",
        "got error envelope: {out}"
    );
...
fn every_diagram_stays_within_the_readability_budget() {
    ...
    let steps: Vec<Step> = (0..8)
        .map(|i| {
            step(
                &format!("n{i}"),
                &format!("Node number {i} with a long descriptive label"),
                &[],
            )
        })
        .collect();
    ...
    if v["status"] == "error" {
        ...
        assert!(
            svg_width(html) <= 1100.0 + 60.0,
            "flowchart exceeds readability budget: {}px",
            svg_width(html)
        );
    }
}
...
fn long_flowchart_labels_stay_inside_their_node_boxes() {
    ...
    let steps: Vec<Step> = vec![
        step(
            "a",
            "An extremely wide node label that must never spill",
            &["b"],
        ),
        step(
            "b",
            "Another very long label that also must not overflow",
            &["c"],
        ),
    ...
    while let Some(rel) = html[scan_from..].find("<text ") {
        ...
        if opening.contains("node-text") && !opening.contains("title-text") {
            ...
            let box_rect = parse_rects(&html).into_iter().find(|r| {
                r.class.contains("node-box")
                    && x >= r.x
                    && x <= r.x + r.w
                    && y >= r.y
                    && y <= r.y + r.h
